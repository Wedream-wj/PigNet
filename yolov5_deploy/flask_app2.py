...
# import detector
# from onnx_inference2 import Yolov5ONNX
from onnx_inference3 import Yolov5ONNX
...
if not os.path.exists(set_upload_path):
    os.makedirs(set_upload_path)
if not os.path.exists(set_result_path):
    os.makedirs(set_result_path)
...
det = Yolov5ONNX(ip_port=ip_port)
...
app.config['UPLOAD_FOLDER'] = os.path.join(basepath,'static/images/upload')
# 设置检测结果保存的文件夹
# app.config['RESULT_FOLDER'] = basepath+r'/static/images/results'
app.config['RESULT_FOLDER'] = os.path.join(basepath,'static/images/results')
...
def get_frame(imageId):
    ...
    with open(os.path.join(basepath,'static','images','results','{}'.format(imageId)), 'rb') as f:
...
def get_yolo():
    ...
    # lab, img, loc, res = yolo5.detect(o_source = upload_path)
    lab, img, loc, res = det.inference(img_path=all_file_name_g)
    basepath = os.path.dirname(__file__)
    ...
    # 检测结果写到的目录
    cv2.imwrite(os.path.join(basepath, set_result_path, det.img_name ), img)
    # print("[test...]res=>", res)
    return res
...
def upload():
    if request.method == 'POST':
        ...
        # lab, img, loc, res = yolo5.detect(o_source = upload_path)
        lab, img, loc, res = det.inference(img_path=upload_path)

        #检测结果写到的目录
        # cv2.imwrite(os.path.join(basepath, set_result_path, filename_+'.jpg'), img)
        # print(os.path.join(basepath, set_result_path, filename_+'.jpg'))
        # print("[test...]res=>",res)
        return res
